for p=2:3 %the moment order of the HMCR measure
    clearvars -except p
    sz=10000; %the number of original scenarios
    prob=ones(1,sz)/sz;
    alpha=0.85;
    redu=[200,600,1000];
    asset_num=50;%the number of assets
    
    orisolution=zeros(20,50);oriobj=zeros(20,1);
    for i=1:20
    ...
    for j=1:3
        ...
        for i=1:20 %20 original scenario sets
            ...
            [scenario,pro]=r_cluster(oriscenario,redu(j),prob);
            [x1,obj1,~]=portfolio(scenario,pro',p,alpha);
            ...
            erro(i,5)=sum(ind==0);
            [scenario,pro]=r_cluster(oriscenario,erro(i,5),prob);
            [x1,obj1,~]=portfolio(scenario,pro',p,alpha);
        ...
        save(['erro-bin',num2str((p-2)*3+j),'.mat'],'result'); 
